...
(function () {
  ...
  var foolslide = {
    ...
    category: 'manga',
    run() {
      const temp = `${W.location.href.slice(
        0,
        W.location.href.lastIndexOf('/')
      )}/`;
      ...
      return {
        ...
        prev: chapter
          .eq(
            chapter.index(
              chapter.filter(
                `[href*='${W.location.pathname.replace(/page.+/, '')}']`
              )
            ) + 1
          )
          .attr('href'),
  ...
  var mangadex = {
    ...
    run() {
      let server = null;
      const chapterId = W.location.pathname.match(
        /\/chapter\/([^/]+)(\/[0-9]+)?/
      )[1];
    ...
    run() {
      ...
      return {
        ...
        listImages: images.map(
          (i) => `https://img.mghubcdn.com/file/imghub/${i}`
        ),
      };
    },
  };
  ...
  var mangakakalot = {
    ...
    run() {
      ...
      return {
        title: $('.info-top-chapter h2, .panel-chapter-info-top h1')
          .text()
          .trim(),
        series: $('span a[title]').eq(1).attr('href'),
        pages: images.length,
        prev: $('.navi-change-chapter-btn-prev:first, .next:first').attr(
          'href'
        ),
        next: $('.navi-change-chapter-btn-next:first, .back:first').attr(
          'href'
        ),
        listImages: images.map((i) => $(i).attr('src')),
      };
  ...
  var mangalyght = {
    ...
    run() {
      ...
      const url = `${$("form[name='pageSelector1']").attr(
        'action'
      )}?ch=${chapter.val().replace(' ', '+')}&page=`;
  ...
  var mangapark = {
    ...
    run() {
      const pass = JSON.parse(
        CryptoJS.AES.decrypt(amWord, amPass).toString(CryptoJS.enc.Utf8)
      );
      return {
        ...
        prev: prevEpiUrl,
        next: nextEpiUrl,
        listImages: imgPathLis.map(
          (i, index) => `${imgCdnHost + i}?${pass[index]}`
        ),
      };
    },
  };
  ...
  var senmanga = {
    ...
    category: 'manga',
    run() {
      const url = `/${W.location.pathname.split('/')[1]}/${
        W.location.pathname.split('/')[2]
      }`;
  ...
  var tmofans = {
    ...
    run() {
      const num =
        $('#viewer-pages-select:first option').get().length ||
        $('.img-container img').get().length;
      return {
        title: $('title').text().trim(),
  ...
  var unionmangas = {
    ...
    run() {
      return {
        ...
        prev: `/leitor/${$('#mangaUrl').text()}/${$('.listCap:selected')
          .prev()
          .val()}`,
        next: `/leitor/${$('#mangaUrl').text()}/${$('.listCap:selected')
          .prev()
          .next()}`,
  ...
  const getValueGM = (name, defaultValue = null) => {
  ...
  const getExtension = (mimeType) =>
    ((/image\/(?<ext>jpe?g|png|webp)/.exec(mimeType) || {}).groups || {}).ext ||
    '' ||
    'png';
  ...
  function generateZip() {
    ...
    if (cache.downloadFiles === 0) {
      ...
      images.get().forEach((value, index) => {
        ...
        const base64 = /^data:(?<mimeType>image\/\w+);base64,+(?<data>.+)/.exec(
          src
        );
        if (base64) {
              ...
              onload(request) {
                const filename =
                  ...
                  getFilename(
                    ...
                    index,
                    images.length,
                    getExtension(request.response.type)
                  );
    ...
    if (cache.downloadFiles < total) {
      logScript(
        `Waiting for Files to Download ${cache.downloadFiles} of ${total}`
      );
      ...
      $(`#PageImg${index}`)
        ...
        .on('loaded.unveil', () => {
          $(`#PageImg${index}`)
            .parent()
            .imagesLoaded()
            .progress(onImagesProgress);
          logScript(
            'Unveiled Image: ',
            index,
            ' Source: ',
            $(`#PageImg${index}`).attr('src')
          );
        });
    }
    return index;
  ...
  const loadMangaPages = (begin, manga) =>
    manga.listPages.map((url, index) =>
      index >= begin
        ? delayAdd(url, (manga.timer || settings.Timer) * (index - begin)).then(
            (response) => addPage(manga, index + 1, response)
          )
        : null
    );
  ...
  const loadMangaImages = (begin, manga) =>
    manga.listImages.map((src, index) =>
      index >= begin
        ? delayAdd(src, (manga.timer || settings.Timer) * (index - begin)).then(
            (response) => addImg(index + 1, response)
          )
        : null
    );
  ...
  function controls$1() {
    ...
    $('#hidePageControls').on('change', (event) => {
      ...
      logScript(
        `MangaHidePageControls: ${getValueGM('MangaHidePageControls')}`
      );
    });
    // Theme Control
    ...
    $('.Bookmark').on('click', (event) => {
      ...
      const found =
        settings.bookmarks.filter((el) => el.url === mark.url).length > 0;
      settings.bookmarks = settings.bookmarks.filter(
        (el) => el.url !== mark.url
      );
      if (found) {
    ...
    $('.ZoomIn').on('click', (event) => {
      const img = $(event.target)
        .parents('.MangaPage')
        .find('.PageContent img');
      const ratio =
        (img.width() / img.prop('naturalWidth')) * (100 + settings.zoomStep);
      applyZoom(img, ratio);
    });
    // ZoomOut
  ...
  <span class='controlLabel PagesPerSecond'>Pages/Second:
    <select id='PagesPerSecond'>
      ...
      <option value='2000' ${
        settings.Timer === 2000 ? 'selected' : ''
      }>0.5</option>
      <option value='1000' ${
        settings.Timer === 1000 ? 'selected' : ''
      }>01(Normal)</option>
      <option value='500' ${
        settings.Timer === 500 ? 'selected' : ''
      }>02</option>
      <option value='250' ${
        settings.Timer === 250 ? 'selected' : ''
      }>04(Fast)</option>
      <option value='125' ${
        settings.Timer === 125 ? 'selected' : ''
      }>08</option>
      <option value='100' ${
  ...
  <span class='controlLabel DefaultZoom'>Default Zoom:
    <select id='DefaultZoom'>
      ...
      <option value='100' ${
        settings.Zoom === 100 ? 'selected' : ''
      }>100%</option>
      <option value='125' ${
        settings.Zoom === 125 ? 'selected' : ''
      }>125%</option>
      <option value='150' ${
        settings.Zoom === 150 ? 'selected' : ''
      }>150%</option>
      <option value='175' ${
        settings.Zoom === 175 ? 'selected' : ''
      }>175%</option>
      <option value='200' ${
        settings.Zoom === 200 ? 'selected' : ''
      }>200%</option>
      <option value='1000' ${
  ...
  const listOptions = (times) =>
    [...Array(times).keys()].map(
      (index) => `<option value='${index + 1}'>${index + 1}</option>`
    );
    ...
    [...Array(times).keys()].map(
      (index) =>
        `<div id='Thumbnail${
          ...
          index + 1
        }' alt='ThumbnailImg${index + 1}' src=''/><span>${
          index + 1
        }</span></div>`
    );
  const body = (manga, begin = 0) => `
<div id='MangaOnlineViewer' class='${settings.Theme} ${
    isMobile ? 'mobile' : ''
  ...
  <div id='Chapter' class='${
    settings.FitWidthIfOversize === true ? 'fitWidthIfOversize' : ''
  } ${settings.viewMode}'>
  ...
  <div id='Navigation' class='panel ${
    ...
    <div id='NavigationCounters' class='controlLabel'>
      <img alt='Thumbnails' title='Thumbnails' src='${
        icon.menu
      }' class='nav' /><i>0</i> of <b>${manga.pages}</b> Pages Loaded
    </div>
    <div id='Thumbnails'>
  ...
  function formatPage(manga, begin) {
    ...
    setTimeout(() => {
      try {
        ...
        if (
          !isNothing(
            settings.bookmarks.filter((el) => el.url === W.location.href)
          )
        ) {
          logScript(`Bookmark Removed ${W.location.href}`);
          settings.bookmarks = settings.bookmarks.filter(
            (el) => el.url !== W.location.href
          );
          setValueGM('MangaBookmarks', JSON.stringify(settings.bookmarks));
        }
      } catch (e) {
  ...
  function preparePage(site, manga, begin = 0) {
    ...
    if (manga.pages > 0) {
      ...
      switch (site.start || settings.loadMode) {
        case 'never':
          $('body').append(
            '<button id="mov" onclick=mov()>Start MangaOnlineViewer</button>'
          );
          break;
        case 'always':
          formatPage(manga);
        ...
        default:
          Swal.fire({
            ...
            html: `${
              beginning > 1
                ? `Resuming reading from Page ${beginning}.<br/>`
                : ''
  ...
  function start(sites) {
    ...
    function waitExec(site) {
      ...
      if (site.waitAttr !== undefined) {
        ...
        logScript(
          `Waiting for ${site.waitAttr[1]} of ${site.waitAttr[0]} = ${wait}`
        );
        if (isNothing(wait)) {
          setTimeout(() => {
            waitExec(site);
