...
// @include /https?:\/\/(www.)?(asura|flamescans|realmscans|alpha-scans|void-scans).(com|org|gg)\/.+/
            ...
            return {
                ...
                before() {
                  ...
                  if (window.location.pathname.match(/\d+-\d+.html$/)) {
                    window.location.pathname = window.location.pathname.replace(/-\d+.html$/,
                      '-all.html');
                  }
                },
            };
...
`);
  ...
  // language=CSS
  const sweetalert = `.swal2-popup.swal2-toast {
...
<div id="Page${index}" class="MangaPage">
  ...
  <div class="PageContent">
    ...
    const localeSelector = locales.map((locale) => `<option value="${locale.ID}" ${useSettings().locale === locale.ID ? 'selected' : ''}>${locale.NAME}</option>`);
    // language=html
    const SettingsPanel = `
