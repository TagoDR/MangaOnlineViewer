...
(function () {
  ...
  const threehentai = {
    ...
    run() {
      return {
        ...
        prev: "#",
        next: "#",
        listImages: Object.keys(unsafeWindow.readerPages.pages).map(
          (img) => unsafeWindow.readerPages.baseUriImg.replace("%s", unsafeWindow.readerPages.pages[img].f)
        )
      };
    }
  };
  ...
  const ninehentai = {
    ...
    async run() {
      ...
      return {
        ...
        prev: "#",
        next: "#",
        listImages: Array(api.results.total_page).fill(0).map(
          (_, i) => `${api.results.image_server.replace(".com", ".so") + api.results.id}/${i + 1}.jpg`
        )
      };
    }
  };
  ...
  const exhentai = {
    ...
    async run() {
      ...
      const fetchBlocks = Array(maxGalley).fill(0).map(
        async (_, galleryId) => fetch(`${gallery}?p=${galleryId}`).then(async (res) => res.text()).then((html) => new DOMParser().parseFromString(html, "text/html"))
      );
      const data = await Promise.all(fetchBlocks);
      const pages = data.flatMap(
        (html) => [...html.querySelectorAll("#gdt a")].map((item) => item.getAttribute("href") ?? "")
      );
      return {
        ...
        async reload(page) {
          ...
          return fetch(newUrl).then((res) => res.text()).then(
            (html) => new DOMParser()?.parseFromString(html, "text/html")?.querySelector("#img")?.getAttribute("src") ?? ""
          );
        }
      };
    }
    ...
    run() {
      return {
        ...
        listImages: unsafeWindow.gData.images.map(
          (i) => `https://static.hentaicdn.com/hentai${i}`
        )
      };
    }
  };
  ...
  const hentaiforce = {
    ...
    run() {
      return {
        ...
        prev: "#",
        next: "#",
        listImages: Array(unsafeWindow.readerPages.lastPage).fill(0).map(
          (_, i) => unsafeWindow.readerPages.baseUriImg.replace("%c", unsafeWindow.readerPages.pages[i + 1].l).replace("%s", unsafeWindow.readerPages.pages[i + 1].f)
        )
      };
    }
  };
  ...
  const hentalk = {
    ...
    async run() {
      ...
      ).then(async (res) => res.json()).then((j) => j.nodes[2].data);
  ...
  const hitomi = {
    ...
    run() {
      return {
        ...
        prev: "#",
        next: "#",
        listImages: unsafeWindow.galleryinfo.files.map(
          (file) => unsafeWindow.url_from_url_from_hash(unsafeWindow.galleryinfo, file, "webp")
        )
      };
    }
  };
  ...
  const imhentai = {
    ...
    async run() {
      ...
      return {
        ...
        prev: "#",
        next: "#",
        listImages: Array(num).fill(0).map(
          (_, i) => `//${randomServer}/${imageDir}/${galleryId}/${i + 1}.${extensionByCode(
  ...
  const multporn = {
    ...
    async run() {
      ...
      const api = await fetch(url).then(async (res) => res.text()).then((html) => new DOMParser().parseFromString(html, "text/xml"));
  ...
  const nhentainet = {
    ...
    run() {
      ...
      const ext = unsafeWindow._gallery?.images?.pages?.map(
        (i) => extensionByCode(i.t)
      );
      return {
      ...
      const src = [
        ...new Set(
          [...document.querySelectorAll("article .page__text img , article #content-2 img")].map(
            (img) => img.getAttribute("data-src") ?? img.getAttribute("data-srce") ?? img.closest("a")?.getAttribute("href") ?? img.getAttribute("src") ?? ""
          )
        )
      ];
      return {
  ...
  function logScriptVerbose(...text) {
    if (["dev", "development"].includes("adult"))
  ...
  const o$d="@moaqzdev/toast",i$6={_dispatchToast(s,t){Object.assign(t,{type:s});const a=new CustomEvent(o$d,{detail:t});document.dispatchEvent(a);},success(s){this._dispatchToast("success",s);},error(s){this._dispatchToast("error",s);},warning(s){this._dispatchToast("warning",s);},info(s){this._dispatchToast("info",s);},confirm(s){this._dispatchToast("confirm",s);}};
   ...
   */
  const t$4=globalThis,i$5=t=>t,s$3=t$4.trustedTypes,e$8=s$3?s$3.createPolicy("lit-html",{createHTML:t=>t}):void 0,h$3="$lit$",o$c=`lit$${Math.random().toFixed(9).slice(2)}$`,n$6="?"+o$c,r$7=`<${n$6}>`,l$1=document,c$3=()=>l$1.createComment(""),a$1=t=>null===t||"object"!=typeof t&&"function"!=typeof t,u$1=Array.isArray,d$1=t=>u$1(t)||"function"==typeof t?.[Symbol.iterator],f$2="[ \t\n\f\r]",v=/<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g,_$1=/-->/g,m=/>/g,p$1=RegExp(`>|${f$2}(?:([^\\s"'>=/]+)(${f$2}*=${f$2}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`,"g"),g=/'/g,$=/"/g,y$1=/^(?:script|style|textarea|title)$/i,x$1=t=>(i,...s)=>({_$litType$:t,strings:i,values:s}),b$1=x$1(1),E=Symbol.for("lit-noChange"),A=Symbol.for("lit-nothing"),C=new WeakMap,P=l$1.createTreeWalker(l$1,129);function V(t,i){if(!u$1(t)||!t.hasOwnProperty("raw"))throw Error("invalid template strings array");return void 0!==e$8?e$8.createHTML(i):i}const N=(t,i)=>{const s=t.length-1,e=[];let n,l=2===i?"<svg>":3===i?"<math>":"",c=v;for(let i=0;i<s;i++){const s=t[i];let a,u,d=-1,f=0;for(;f<s.length&&(c.lastIndex=f,u=c.exec(s),null!==u);)f=c.lastIndex,c===v?"!--"===u[1]?c=_$1:void 0!==u[1]?c=m:void 0!==u[2]?(y$1.test(u[2])&&(n=RegExp("</"+u[2],"g")),c=p$1):void 0!==u[3]&&(c=p$1):c===p$1?">"===u[0]?(c=n??v,d=-1):void 0===u[1]?d=-2:(d=c.lastIndex-u[2].length,a=u[1],c=void 0===u[3]?p$1:'"'===u[3]?$:g):c===$||c===g?c=p$1:c===_$1||c===m?c=v:(c=p$1,n=void 0);const x=c===p$1&&t[i+1].startsWith("/>")?" ":"";l+=c===v?s+r$7:d>=0?(e.push(a),s.slice(0,d)+h$3+s.slice(d)+o$c+x):s+o$c+(-2===d?i:x);}return [V(t,l+(t[s]||"<?>")+(2===i?"</svg>":3===i?"</math>":"")),e]};let S$1 = class S{constructor({strings:t,_$litType$:i},e){let r;this.parts=[];let l=0,a=0;const u=t.length-1,d=this.parts,[f,v]=N(t,i);if(this.el=S.createElement(f,e),P.currentNode=this.el.content,2===i||3===i){const t=this.el.content.firstChild;t.replaceWith(...t.childNodes);}for(;null!==(r=P.nextNode())&&d.length<u;){if(1===r.nodeType){if(r.hasAttributes())for(const t of r.getAttributeNames())if(t.endsWith(h$3)){const i=v[a++],s=r.getAttribute(t).split(o$c),e=/([.?@])?(.*)/.exec(i);d.push({type:1,index:l,name:e[2],strings:s,ctor:"."===e[1]?I:"?"===e[1]?L:"@"===e[1]?z:H}),r.removeAttribute(t);}else t.startsWith(o$c)&&(d.push({type:6,index:l}),r.removeAttribute(t));if(y$1.test(r.tagName)){const t=r.textContent.split(o$c),i=t.length-1;if(i>0){r.textContent=s$3?s$3.emptyScript:"";for(let s=0;s<i;s++)r.append(t[s],c$3()),P.nextNode(),d.push({type:2,index:++l});r.append(t[i],c$3());}}}else if(8===r.nodeType)if(r.data===n$6)d.push({type:2,index:l});else {let t=-1;for(;-1!==(t=r.data.indexOf(o$c,t+1));)d.push({type:7,index:l}),t+=o$c.length-1;}l++;}}static createElement(t,i){const s=l$1.createElement("template");return s.innerHTML=t,s}};function M(t,i,s=t,e){if(i===E)return i;let h=void 0!==e?s._$Co?.[e]:s._$Cl;const o=a$1(i)?void 0:i._$litDirective$;return h?.constructor!==o&&(h?._$AO?.(false),void 0===o?h=void 0:(h=new o(t),h._$AT(t,s,e)),void 0!==e?(s._$Co??=[])[e]=h:s._$Cl=h),void 0!==h&&(i=M(t,h._$AS(t,i.values),h,e)),i}class R{constructor(t,i){this._$AV=[],this._$AN=void 0,this._$AD=t,this._$AM=i;}get parentNode(){return this._$AM.parentNode}get _$AU(){return this._$AM._$AU}u(t){const{el:{content:i},parts:s}=this._$AD,e=(t?.creationScope??l$1).importNode(i,true);P.currentNode=e;let h=P.nextNode(),o=0,n=0,r=s[0];for(;void 0!==r;){if(o===r.index){let i;2===r.type?i=new k(h,h.nextSibling,this,t):1===r.type?i=new r.ctor(h,r.name,r.strings,this,t):6===r.type&&(i=new Z(h,this,t)),this._$AV.push(i),r=s[++n];}o!==r?.index&&(h=P.nextNode(),o++);}return P.currentNode=l$1,e}p(t){let i=0;for(const s of this._$AV) void 0!==s&&(void 0!==s.strings?(s._$AI(t,s,i),i+=s.strings.length-2):s._$AI(t[i])),i++;}}class k{get _$AU(){return this._$AM?._$AU??this._$Cv}constructor(t,i,s,e){this.type=2,this._$AH=A,this._$AN=void 0,this._$AA=t,this._$AB=i,this._$AM=s,this.options=e,this._$Cv=e?.isConnected??true;}get parentNode(){let t=this._$AA.parentNode;const i=this._$AM;return void 0!==i&&11===t?.nodeType&&(t=i.parentNode),t}get startNode(){return this._$AA}get endNode(){return this._$AB}_$AI(t,i=this){t=M(this,t,i),a$1(t)?t===A||null==t||""===t?(this._$AH!==A&&this._$AR(),this._$AH=A):t!==this._$AH&&t!==E&&this._(t):void 0!==t._$litType$?this.$(t):void 0!==t.nodeType?this.T(t):d$1(t)?this.k(t):this._(t);}O(t){return this._$AA.parentNode.insertBefore(t,this._$AB)}T(t){this._$AH!==t&&(this._$AR(),this._$AH=this.O(t));}_(t){this._$AH!==A&&a$1(this._$AH)?this._$AA.nextSibling.data=t:this.T(l$1.createTextNode(t)),this._$AH=t;}$(t){const{values:i,_$litType$:s}=t,e="number"==typeof s?this._$AC(t):(void 0===s.el&&(s.el=S$1.createElement(V(s.h,s.h[0]),this.options)),s);if(this._$AH?._$AD===e)this._$AH.p(i);else {const t=new R(e,this),s=t.u(this.options);t.p(i),this.T(s),this._$AH=t;}}_$AC(t){let i=C.get(t.strings);return void 0===i&&C.set(t.strings,i=new S$1(t)),i}k(t){u$1(this._$AH)||(this._$AH=[],this._$AR());const i=this._$AH;let s,e=0;for(const h of t)e===i.length?i.push(s=new k(this.O(c$3()),this.O(c$3()),this,this.options)):s=i[e],s._$AI(h),e++;e<i.length&&(this._$AR(s&&s._$AB.nextSibling,e),i.length=e);}_$AR(t=this._$AA.nextSibling,s){for(this._$AP?.(false,true,s);t!==this._$AB;){const s=i$5(t).nextSibling;i$5(t).remove(),t=s;}}setConnected(t){ void 0===this._$AM&&(this._$Cv=t,this._$AP?.(t));}}class H{get tagName(){return this.element.tagName}get _$AU(){return this._$AM._$AU}constructor(t,i,s,e,h){this.type=1,this._$AH=A,this._$AN=void 0,this.element=t,this.name=i,this._$AM=e,this.options=h,s.length>2||""!==s[0]||""!==s[1]?(this._$AH=Array(s.length-1).fill(new String),this.strings=s):this._$AH=A;}_$AI(t,i=this,s,e){const h=this.strings;let o=false;if(void 0===h)t=M(this,t,i,0),o=!a$1(t)||t!==this._$AH&&t!==E,o&&(this._$AH=t);else {const e=t;let n,r;for(t=h[0],n=0;n<h.length-1;n++)r=M(this,e[s+n],i,n),r===E&&(r=this._$AH[n]),o||=!a$1(r)||r!==this._$AH[n],r===A?t=A:t!==A&&(t+=(r??"")+h[n+1]),this._$AH[n]=r;}o&&!e&&this.j(t);}j(t){t===A?this.element.removeAttribute(this.name):this.element.setAttribute(this.name,t??"");}}class I extends H{constructor(){super(...arguments),this.type=3;}j(t){this.element[this.name]=t===A?void 0:t;}}class L extends H{constructor(){super(...arguments),this.type=4;}j(t){this.element.toggleAttribute(this.name,!!t&&t!==A);}}class z extends H{constructor(t,i,s,e,h){super(t,i,s,e,h),this.type=5;}_$AI(t,i=this){if((t=M(this,t,i,0)??A)===E)return;const s=this._$AH,e=t===A&&s!==A||t.capture!==s.capture||t.once!==s.once||t.passive!==s.passive,h=t!==A&&(s===A||e);e&&this.element.removeEventListener(this.name,this,s),h&&this.element.addEventListener(this.name,this,t),this._$AH=t;}handleEvent(t){"function"==typeof this._$AH?this._$AH.call(this.options?.host??this.element,t):this._$AH.handleEvent(t);}}class Z{constructor(t,i,s){this.element=t,this.type=6,this._$AN=void 0,this._$AM=i,this.options=s;}get _$AU(){return this._$AM._$AU}_$AI(t){M(this,t);}}const B=t$4.litHtmlPolyfillSupport;B?.(S$1,k),(t$4.litHtmlVersions??=[]).push("3.3.2");const D=(t,i,s)=>{const e=s?.renderBefore??i;let h=e._$litPart$;if(void 0===h){const t=s?.renderBefore??null;e._$litPart$=h=new k(i.insertBefore(c$3(),t),t,void 0,s??{});}return h._$AI(t),h};

  /**
   * @license
   * Copyright 2020 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const r$6=o=>void 0===o.strings;
   ...
   * SPDX-License-Identifier: BSD-3-Clause
   */
  const t$3={ATTRIBUTE:1,CHILD:2},e$7=t=>(...e)=>({_$litDirective$:t,values:e});let i$4 = class i{constructor(t){}get _$AU(){return this._$AM._$AU}_$AT(t,e,i){this._$Ct=t,this._$AM=e,this._$Ci=i;}_$AS(t,e){return this.update(t,e)}update(t,e){return this.render(...e)}};

  /**
   * @license
   * Copyright 2017 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const s$2=(i,t)=>{const e=i._$AN;if(void 0===e)return  false;for(const i of e)i._$AO?.(t,false),s$2(i,t);return  true},o$b=i=>{let t,e;do{if(void 0===(t=i._$AM))break;e=t._$AN,e.delete(i),i=t;}while(0===e?.size)},r$5=i=>{for(let t;t=i._$AM;i=t){let e=t._$AN;if(void 0===e)t._$AN=e=new Set;else if(e.has(i))break;e.add(i),c$2(t);}};function h$2(i){ void 0!==this._$AN?(o$b(this),this._$AM=i,r$5(this)):this._$AM=i;}function n$5(i,t=false,e=0){const r=this._$AH,h=this._$AN;if(void 0!==h&&0!==h.size)if(t)if(Array.isArray(r))for(let i=e;i<r.length;i++)s$2(r[i],false),o$b(r[i]);else null!=r&&(s$2(r,false),o$b(r));else s$2(this,i);}const c$2=i=>{i.type==t$3.CHILD&&(i._$AP??=n$5,i._$AQ??=h$2);};let f$1 = class f extends i$4{constructor(){super(...arguments),this._$AN=void 0;}_$AT(i,t,e){super._$AT(i,t,e),r$5(this),this.isConnected=i._$AU;}_$AO(i,t=true){i!==this.isConnected&&(this.isConnected=i,i?this.reconnected?.():this.disconnected?.()),t&&(s$2(this,i),o$b(this));}setValue(t){if(r$6(this._$Ct))this._$Ct._$AI(t,this);else {const i=[...this._$Ct._$AH];i[this._$Ci]=t,this._$Ct._$AI(i,this,0);}}disconnected(){}reconnected(){}};

  /**
   * @license
   * Copyright 2020 Google LLC
   * SPDX-License-Identifier: BSD-3-Clause
   */const e$6=()=>new h$1;let h$1 = class h{};const o$a=new WeakMap,n$4=e$7(class extends f$1{render(i){return A}update(i,[s]){const e=s!==this.G;return e&&void 0!==this.G&&this.rt(void 0),(e||this.lt!==this.ct)&&(this.G=s,this.ht=i.options?.host,this.rt(this.ct=i.element)),A}rt(t){if(this.isConnected||(t=void 0),"function"==typeof this.G){const i=this.ht??globalThis;let s=o$a.get(i);void 0===s&&(s=new WeakMap,o$a.set(i,s)),void 0!==s.get(this.G)&&this.G.call(this.ht,void 0),s.set(this.G,t),void 0!==t&&this.G.call(this.ht,t);}else this.G.value=t;}get lt(){return "function"==typeof this.G?o$a.get(this.ht??globalThis)?.get(this.G):this.G?.value}disconnected(){this.lt===this.ct&&this.rt(void 0);}reconnected(){this.rt(this.ct);}});
  ...
  function areBookmarksEqual(value, other) {
    if (Array.isArray(value) && Array.isArray(other)) {
      ...
      const sortedValue = [...value].sort(
        (a, b) => getBookmarkSortKey(a).localeCompare(getBookmarkSortKey(b))
      );
  ...
  function compareSettingsCustomizer(value, other, key) {
    if (key === "bookmarks") {
      return areBookmarksEqual(value, other);
    }
    if (key === "keybinds") {
  ...
  let localSettings = _.defaultsDeep(
    getLocalSettings(getDefault(false)),
    getDefault(false)
  );
  const isSettingsLocal = () => localSettings?.enabled === true;
  const isLocalSettingsAllowed = (key) => isSettingsLocal() && !["locale", "bookmarks", "keybinds"].includes(key);
  ...
  function parseCss(css) {
    const ruleRegex = /([^{}]+)\s*\{([^}]+)\}/g;
    return [...css.matchAll(ruleRegex)].map((match) => {
  ...
  const styledIcons = Object.fromEntries(
    Object.keys(rawIcons).map((iconKey) => {
      const kebabCaseName = iconKey.replace(/^Icon/, "").replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase();
   ...
   * SPDX-License-Identifier: BSD-3-Clause
   */const e=e$7(class extends i$4{constructor(t){if(super(t),t.type!==t$3.ATTRIBUTE||"class"!==t.name||t.strings?.length>2)throw Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.")}render(t){return " "+Object.keys(t).filter(s=>t[s]).join(" ")+" "}update(s,[i]){if(void 0===this.st){this.st=new Set,void 0!==s.strings&&(this.nt=new Set(s.strings.join(" ").split(/\s/).filter(t=>""!==t)));for(const t in i)i[t]&&!this.nt?.has(t)&&this.st.add(t);return this.render(i)}const r=s.element.classList;for(const t of this.st)t in i||(r.remove(t),this.st.delete(t));for(const t in i){const s=!!i[t];s===this.st.has(t)||this.nt?.has(t)||(s?(r.add(t),this.st.add(t)):(r.remove(t),this.st.delete(t)));}return E}});
  ...
  let Button = class extends i$1 {
    ...
    handleLabelSlotChange() {
      ...
      const isIcon = (el) => ["wa-icon", "mov-icon", "svg"].includes(el.localName);
  ...
  let ColorPicker = class extends i$1 {
    ...
    renderPickerBody() {
      ...
      <div class="swatches">
        ${(this.swatches || Object.entries(colors).filter(([name]) => !["dark", "gray", "zinc", "neutral", "stone"].includes(name)).map(([, color]) => color["600"])).map(
      (color) => b$1`
            <button
              class="swatch"
              title=${color}
    ...
    render() {
      ...
      if (this.mode === "popup") {
        ...
        ${this.opened ? b$1`<div class=${e(pickerClasses)}>${pickerBody}</div>` : ""}
      `;
      }
      return b$1`<div class=${e(pickerClasses)}>${pickerBody}</div>`;
    }
    parseColor(color) {
      try {
  ...
  let SegmentedControl = class extends i$1 {
    ...
    updated(changedProperties) {
      ...
      if (changedProperties.has("value") || changedProperties.has("_options") || changedProperties.has("labelPosition") || changedProperties.has("size")) {
  ...
  let ToggleSwitch = class extends i$1 {
    ...
    render() {
      ...
      if (this.design === "graphical") {
        knobContent = b$1`${this.checked ? IconCheck : IconX}`;
      } else {
        knobContent = b$1`<span class="text">${this.checked ? this.textOn : this.textOff}</span>`;
      }
      return b$1`
      <input
        type="checkbox"
  ...
  class HeadroomController {
    constructor(host) {
      ...
      this.handleScroll = _.throttle(() => {
        ...
        if (getSettingsValue("zoomMode") !== "height" && scrollY + window.innerHeight + showEnd > document.body.scrollHeight) {
          newHeadroom = "end";
    ...
    static isMouseInsideRegion(event, headerWidth, headerHeight2) {
      return event.clientX >= 0 && event.clientX <= headerWidth && event.clientY >= 0 && event.clientY <= headerHeight2;
    }
  }
  ...
  class TitleController {
    ...
    update() {
      ...
      for (let i = 1; i < fullText.length; i++) {
        ...
        if (this.canvasContext.measureText(startCandidate).width + this.canvasContext.measureText(endCandidate).width > targetWidth) {
          break;
        }
  ...
  let KeybindingsPanel = class extends i$1 {
    ...
    keybindList() {
      ...
      return Object.keys(keybinds).map((kb) => {
        const keys = keybinds[kb]?.length ? o$1(
          keybinds[kb]?.map((key) => b$1`<kbd class="dark">${key}</kbd>`),
          " / "
        ) : "";
    ...
    keybindEditor() {
      ...
      return Object.keys(keybinds).map(
        (kb) => b$1`<label for="${kb}">${getLocaleString(kb)}:</label>
          <input
            type="text"
            class="KeybindInput"
            id="${kb}"
  ...
  const SettingsPanelGeneral = () => b$1`${settingsScope()} ${language()}`;
  ...
  const SettingsPanelLoading = () => b$1`${loadMode()} ${loadSpeed()}`;
  ...
  const SettingsPanelOthers = () => b$1`${checkboxOptions()} ${pagination()} ${lazyLoad()} ${headerType()} ${navbarType()} ${autoScroll()}`;
  ...
  function loadMangaPages(begin, manga) {
    sequence(manga.pages, begin).filter(
      (_index, position) => !(manga.lazy ?? getSettingsValue("lazyLoadImages")) || position <= getSettingsValue("lazyStart")
  ...
  const updateOnResize = _.debounce(() => {
  ...
  function buttonZoomWidth(event) {
    ...
    if (img) {
      setAppStateValue("images", {
        ...
        [index]: {
          ...img,
          width: window.innerWidth + (getSettingsValue("navbar") === "left" || getSettingsValue("navbar") === "right" ? -navbarSize : 0),
          height: void 0
        }
      });
    }
  ...
  const listPages = (times, begin) => sequence(times, begin).map((index) => {
...
`;
  ...
  */
  (()=>{var E=s=>s==null?false:typeof s=="boolean"?s:s.trim().toLowerCase()==="true";window.tcRangeSliderPlugins=window.tcRangeSliderPlugins||[];var F="min-label",H="max-label",y="#1E293B",A=()=>{let s=null,b=null,i=null,L,m=false,n=y,l="",x,r=null,o=null,u=null,g=[],S=()=>{var t;let e=(t=s==null?void 0:s.shadowRoot)==null?void 0:t.querySelector(".range-slider-box");r=document.createElement("div"),r.classList.add("labels-row"),e.prepend(r);},v=e=>{let t=document.createElement("label");return t.className=e,t.setAttribute("for","range-slider"),t},C=()=>{o=v(F),o.textContent=f(i==null?void 0:i.getTextMin(),l),u=v(H),u.textContent=f(i==null?void 0:i.getTextMax(),l),b==null||b.before(o),b==null||b.after(u);let e=i==null?void 0:i.getValues();if(!!e)for(let t=0;t<e.length;t++){let a=v(`value${t+1}-label generated-label`);a.textContent=f(e[t],l),g.push(a),r==null||r.append(a);}},I=()=>{for(let e of g)!e||e.remove();o==null||o.remove(),u==null||u.remove(),r==null||r.remove(),g=[];},M=e=>{m=e,m?(S(),C()):I();},T=e=>{n=e;for(let t of g)!t||(t.style.color=n!=null?n:y);o&&(o.style.color=n!=null?n:y),u&&(u.style.color=n!=null?n:y);},h=e=>{l=e,L&&L();},P=()=>{!i||!r||r.classList.toggle("is-reversed",i.isRightToLeft()||i.isBottomToTop());},f=(e,t)=>{let a=`${(e!=null?e:"").toString()}${t}`;return !!x&&typeof x=="function"?x(e):a};return {get name(){return "Generated Labels"},init:(e,t,a,d)=>{var c,p,w;s=e,i=d,b=(c=e.shadowRoot)==null?void 0:c.getElementById("range-slider"),L=t,n=(p=s.getAttribute("generate-labels-text-color"))!=null?p:y,l=(w=s.getAttribute("generate-labels-units"))!=null?w:"",M(E(s.getAttribute("generate-labels"))),P(),T(n);},update:e=>{if(!(!m||!e.values)){P();for(let t=0;t<e.values.length;t++){let a=e.values[t],d=g[t];if(a===void 0&&!!d){d.remove(),g[t]=void 0;continue}if(a!==void 0&&!d){let c=v(`value${t+1}-label generated-label`);if(c.textContent=f(a,l),g[t]=c,e.values.length<=0)r==null||r.append(c);else if(t===0)r==null||r.append(c);else {let p=g[t-1];p==null||p.after(c);}continue}!d||(d.textContent=f(a,l));}o&&(o.textContent=f(e.textMin,l)),u&&(u.textContent=f(e.textMax,l)),T(n);}},onAttrChange:(e,t)=>{e==="generate-labels"&&M(E(t)),e==="generate-labels-text-color"&&T(t),e==="generate-labels-units"&&h(t);},gettersAndSetters:[{name:"generateLabels",attributes:{get(){return m!=null?m:false},set:e=>{M(E(e));}}},{name:"textColor",attributes:{get(){return n!=null?n:""},set:e=>{T(e);}}},{name:"generateLabelsTextColor",attributes:{get(){return n!=null?n:""},set:e=>{T(e);}}},{name:"units",attributes:{get(){return l!=null?l:""},set:e=>{h(e);}}},{name:"generateLabelsUnits",attributes:{get(){return l!=null?l:""},set:e=>{h(e);}}},{name:"generateLabelsFormat",attributes:{get(){return x},set:e=>{x=e,L&&L();}}}],css:`
    ...
    `,destroy:I}};window.tcRangeSliderPlugins.push(A);})();
  ...
  Copyright (c) 2022-present, Miriam Zusin                         
  */
  (()=>{var R=(r,n,t,c,d)=>{let a=n-r;return a===0?t:(c-t)*(d-r)/a+t},I=r=>!isNaN(parseFloat(r))&&isFinite(r),g=(r,n)=>I(r)?Number(r):n;var h=r=>r==null?false:typeof r=="boolean"?r:r.trim().toLowerCase()==="true";window.tcRangeSliderPlugins=window.tcRangeSliderPlugins||[];var p=11,y=11,A=()=>{let r=null,n=null,t=null,c=null,d=null,a=false,s=p,u=y,L=()=>{var l;let e=(l=r==null?void 0:r.shadowRoot)==null?void 0:l.querySelector("#range-slider");t=document.createElement("div"),t.classList.add("marks"),c=document.createElement("div"),c.classList.add("mark-points"),t.append(c),d=document.createElement("div"),d.classList.add("mark-values"),t.append(d),e.append(t);},P=()=>{!n||!t||t.classList.toggle("is-reversed",n.isRightToLeft()||n.isBottomToTop());},S=()=>{var v;if(!t||!n)return;let e=n.getMin(),l=n.getMax(),E=n.getType()==="vertical",f=n.isRightToLeft()||n.isBottomToTop();for(let i=0;i<s;i++){let o=document.createElement("div");o.classList.add("mark",`mark-${i}`);let m=s===0?0:i*100/(s-1);E?f?o.style.top=`${100-m}%`:o.style.top=`${m}%`:f?o.style.left=`${100-m}%`:o.style.left=`${m}%`,c==null||c.append(o);}let b=n.getData();for(let i=0;i<u;i++){let o=document.createElement("div");o.classList.add("mark-value",`mark-value-${i}`);let m=u===0?0:i*100/(u-1),w=R(0,u-1,e,l,i);o.textContent=(b?(v=b[Math.round(w)])!=null?v:"":w).toString(),E?f?o.style.top=`${100-m}%`:o.style.top=`${m}%`:f?o.style.left=`${100-m}%`:o.style.left=`${m}%`,d==null||d.append(o);}},k=(e,l)=>{T(),s=e,u=l,s<=0&&(s=p),u<=0&&(u=y),L(),S(),P();},C=e=>{a=e,a?(L(),S(),P()):T();},x=e=>{!t||t.style.setProperty("--marks-color",e);},M=e=>{!t||t.style.setProperty("--values-color",e);},T=()=>{t==null||t.remove();};return {get name(){return "Marks"},init:(e,l,E,f)=>{var b,v;n=f,r=e,a=h(r.getAttribute("marks")),a&&(k(g(r.getAttribute("marks-count"),p),g(r.getAttribute("marks-values-count"),y)),x((b=r.getAttribute("marks-color"))!=null?b:"#cbd5e1"),M((v=r.getAttribute("marks-values-color"))!=null?v:"#475569"));},onAttrChange:(e,l)=>{e==="marks"&&C(h(l)),e==="marks-count"&&k(g(l,p),u),e==="marks-values-count"&&k(s,g(l,y)),e==="marks-color"&&x(l),e==="marks-values-color"&&M(l);},gettersAndSetters:[{name:"marksEnabled",attributes:{get(){return a!=null?a:false},set:e=>{C(h(e));}}},{name:"marksCount",attributes:{get(){return s!=null?s:p},set:e=>{k(g(e,p),u);}}},{name:"marksValuesCount",attributes:{get(){return s!=null?s:p},set:e=>{k(s,g(e,y));}}},{name:"marksColor",attributes:{get(){return t==null?void 0:t.style.getPropertyValue("--marks-color")},set:e=>{x(e);}}},{name:"markValuesColor",attributes:{get(){return t==null?void 0:t.style.getPropertyValue("--values-color")},set:e=>{M(e);}}}],destroy:T,css:`
:root{
  --marks-color: #cbd5e1;
  --values-color: #475569;
}
...
.type-vertical .mark-value{
  ...
  */
  ...
  ], MovStartup.prototype, "mangaPages", 2);
  __decorateClass$1([
    n$1({ type: Number, reflect: true })
  ], MovStartup.prototype, "begin", 2);
  __decorateClass$1([
    n$1({ type: Number })
  ], MovStartup.prototype, "timeoutMs", 2);
  __decorateClass$1([
    n$1({ type: String, reflect: true })
  ], MovStartup.prototype, "status", 2);
  MovStartup = __decorateClass$1([
    t$1("script-startup")
  ], MovStartup);
    ...
    render() {
      ...
      >
        ${manga ? b$1`
              ...
              </div>` : b$1(_a || (_a = __template([' <script-startup\n              .mangaPages="', '"\n              begin="', '"\n              initialStatus="', '"\n              @start=', "\n            ><\/script-startup>"])), this.manga?.pages, this.manga?.begin, r(this.loadMode, [
      ["wait", () => "initial-prompt"],
  ...
  __decorateClass([
    n$1({ type: String, reflect: true })
  ], App.prototype, "loadMode", 2);
  __decorateClass([
    n$1({ type: Object })
  ], App.prototype, "manga", 2);
  App = __decorateClass([
    t$1("manga-online-viewer"),
    libExports.useStores(settings$1, locale, appState)
  ], App);

  const TOAST_EVENT="@moaqzdev/toast";class Toaster extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"});}static#t=3e3;parseDuration(o){if(o==="none"||Number.isInteger(o)&&o>Toaster.#t)return o;const e=this.getAttribute("duration");if(e){const r=Number.parseInt(e);if(!Number.isNaN(r))return r}return Toaster.#t}async createToast({title:o,type:e,description:r,onConfirm:m,onCancel:f,confirmText:v="\u2705",cancelText:b="\u274C",duration:n}){const a=this.shadowRoot.querySelector("#toast-tmpl").content.cloneNode(true),t={container:a.querySelector("[data-toast]"),title:a.querySelector("[data-title]"),description:a.querySelector("[data-description]"),actions:a.querySelector("[data-actions]"),confirmBtn:a.querySelector("button[data-action-type='confirm']"),cancelBtn:a.querySelector("button[data-action-type='cancel']"),closeBtn:a.querySelector("[data-close-button]")};n=this.parseDuration(n),t.title.textContent=o||"",t.container.setAttribute("data-type",e),r==null?t.description?.remove():t.description.textContent=r;const s=()=>this.removeToast(t.container);if(e==="confirm"?(t.confirmBtn.textContent=v,t.confirmBtn.addEventListener("click",()=>{m?.(),s();},{once:true}),t.cancelBtn.textContent=b,t.cancelBtn.addEventListener("click",()=>{f?.(),s();},{once:true})):t.actions?.remove(),this.hasAttribute("dismissable")?t.closeBtn.addEventListener("click",s,{once:true}):t.closeBtn?.remove(),this.shadowRoot.querySelector("[data-toaster]").appendChild(a),n!=="none"){const c=new AbortController,h=Date.now();let i=null,l=0;const u=()=>{c.abort(),s();};let p=setTimeout(u,n);const g=()=>{i==null&&(clearTimeout(p),i=Date.now());},y=()=>{i!=null&&(l=i-h,i=null,p=setTimeout(u,Math.max(n-l,0)));};["focusin","pointerenter","mouseenter"].forEach(d=>{t.container.addEventListener(d,g,{signal:c.signal});}),["focusout","pointerleave","mouseleave"].forEach(d=>{t.container.addEventListener(d,y,{signal:c.signal});});}}removeToast(o){o.animate([{opacity:1},{opacity:0}],{duration:300,easing:"ease",fill:"forwards"}).finished.then(()=>o.remove());}handleEvent(o){if(o instanceof CustomEvent&&o.type===TOAST_EVENT){const e=o.detail;this.createToast(e);}}connectedCallback(){this.render(),document.addEventListener(TOAST_EVENT,this);}disconnectedCallback(){document.removeEventListener(TOAST_EVENT,this);}render(){this.shadowRoot.innerHTML=`
