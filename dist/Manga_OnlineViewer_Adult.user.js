...
(function () {
    ...
    constructor({ strings: t, _$litType$: s }, n) {
      ...
      for (; null !== (r = C.nextNode()) && d.length < u; ) {
        if (1 === r.nodeType) {
          ...
          if (r.data === o$7) d.push({ type: 2, index: c });
          else {
            let t = -1;
  ...
  SegmentedControl = __decorateClass$4([t$2('mov-segmented-control')], SegmentedControl);
  ...
  ToggleSwitch = __decorateClass$3([t$2('mov-toggle-switch')], ToggleSwitch);
  ...
  appState.listen((value, _oldValue, changedKey) => {
    if (changedKey === 'scrollToPage' && value.scrollToPage !== void 0) {
      if (value.scrollToPage <= 0) {
        ...
        scrollToElement(getAppStateValue('render')?.querySelector(`#Page${value.scrollToPage}`));
      }
  ...
  let Pagination = class extends i$2 {
    constructor() {
      super(...arguments);
      this.currentPage = 1;
      this.totalPages = 1;
  ...
  Pagination = __decorateClass$2([t$2('mov-pagination')], Pagination);
  ...
  function applyZoom(mode = getSettingsValue('zoomMode'), value = getSettingsValue('zoomValue')) {
    ...
    for (let i = manga?.begin ?? 1; i <= (manga?.pages ?? 1); i++) {
      ...
      if (mode === 'width') {
        ...
        page.width = page.naturalWidth ? page.naturalWidth * (value / 100) : void 0;
        page.height = void 0;
      }
      newImages[i] = page;
    }
...
`;
  ...
  const keybindEditor = () =>
    Object.keys(getSettingsValue('keybinds'))
      ...
      .concat(x$1` <div id="HotKeysRules">${o$4(getLocaleString('KEYBIND_RULES'))}</div>`);
