...
`;
  ...
  function rectangleRuler(width, height, bgColor, textColor) {
    ...
    for (let x = 0; x <= width; x += 5) {
      const tick = html$1` <line x1="${x}" y1="0" x2="${x}" y2="${rulerMarkerLength(x)}" />`;
      markers += tick;
      if (x !== 0 && x % 50 === 0) {
        const label = html$1` <text
        x="${x}"
        y="25"
        text-anchor="middle"
    ...
    for (let y = 0; y <= height; y += 5) {
      const tick = html$1` <line x1="0" y1="${y}" x2="${rulerMarkerLength(y)}" y2="${y}" />`;
      markers += tick;
      if (y !== 0 && y % 50 === 0) {
        const label = html$1` <text
        x="25"
...
`;
  ...
  function SettingsPanel_General($$anchor, $$props) {
    ...
    template_effect(
      ($0, $1) => {
        ...
        if (select_value !== (select_value = $settings().locale)) {
          ((select.value = (select.__value = $settings().locale) ?? ''),
            select_option(select, $settings().locale));
        }
      },
  ...
  function SettingsPanel_Loading($$anchor, $$props) {
    ...
    template_effect(() => {
      ...
      if (select_value !== (select_value = $settings().loadMode)) {
        ((select.value = (select.__value = $settings().loadMode) ?? ''),
          select_option(select, $settings().loadMode));
      }
      ...
      if (select_1_value !== (select_1_value = $settings().throttlePageLoad)) {
        ((select_1.value = (select_1.__value = $settings().throttlePageLoad) ?? ''),
          select_option(select_1, $settings().throttlePageLoad));
      }
    });
  ...
  function SettingsPanel_Others($$anchor, $$props) {
    ...
    ${$settings().lazyLoadImages ? 'show' : ''}`,
      ...
      if (select_value !== (select_value = $settings().header)) {
        ((select.value = (select.__value = $settings().header) ?? ''),
          select_option(select, $settings().header));
      }
    });
  ...
  var root$5 = from_html(
    `<div class="ControlLabel ColorSchemeSelector"><label> </label> <button class="ControlButton" id="ColorScheme"><!> <!></button></div> <div class="ControlLabel ThemeSelector"><label> </label> <span title="custom"><!> <!></span> <!></div> <div id="Hue"><label> </label> <input class="colorpicker CustomTheme" id="CustomThemeHue" type="color"/></div> <div id="Shade"><span><label> </label> <output class="RangeValue" for="ThemeShade" id="themeShadeVal"> </output></span> <input id="ThemeShade" max="900" min="100" name="ThemeShade" step="100" type="range"/></div>`,
    1,
  );
  ...
  function SettingsPanel_Theme($$anchor, $$props) {
    ...
    var label_3 = child(span_2);
    var text_3 = child(label_3);
  ...
  function SettingsPanel_Zoom($$anchor, $$props) {
    ...
    select_1.__change = function (...$$args) {
      changeViewMode?.apply(this, $$args);
    };
    ...
    template_effect(() => {
      ...
      if (select_value !== (select_value = $settings().zoomMode)) {
        ((select.value = (select.__value = $settings().zoomMode) ?? ''),
          select_option(select, $settings().zoomMode));
      }
      ...
      if (select_1_value !== (select_1_value = $settings().viewMode)) {
        ((select_1.value = (select_1.__value = $settings().viewMode) ?? ''),
          select_option(select_1, $settings().viewMode));
      }
    });
...
`;
  ...
  const listOptions = (times, begin) =>
    indexList(times, begin).map(index => html$1` <option value="${index}">${index}</option>`);
...
`;
  ...
  const site = localhost.run();
  allowUpload();
  document.querySelector('#test')?.addEventListener('click', () => display(site));
  document.querySelector('#testFramework')?.addEventListener('click', () => loadReader(site));
