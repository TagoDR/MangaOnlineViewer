...
(function () {
  ...
  function waitFor(fn, target = document.body) {
    return new Promise((resolve) => {
      const result = fn();
      if (result) {
        resolve(result);
  ...
  const doujinmoe = {
    ...
    run() {
      ...
      return {
        title: document
          .querySelector(".folder-title a:last-child")
          ?.textContent?.trim(),
        series: document
          .querySelector(".folder-title a:nth-last-child(2)")
          ?.getAttribute("href"),
        pages: images.length,
        prev: "#",
        next: "#",
  ...
  const eightMuses = {
    ...
    category: "hentai",
    async run() {
      const img = unsafeWindow.link_images?.slice(
        1,
        unsafeWindow.link_images.length,
      );
      const num =
        img?.length ??
        parseInt(
      ...
      return {
        title: [
          ...document.querySelectorAll(
            ".top-menu-breadcrumb li:not(:last-child)",
          ),
        ]
        ...
        async before() {
          if (!unsafeWindow.link_images?.length) {
            ...
            while (
              document
                .querySelector(".c-dropdown-toggle")
                ?.textContent?.match(/\d+/)
                ?.at(0) !== "1"
            ) {
  ...
  const exhentai = {
    ...
    async run() {
      ...
      return {
        ...
        pages: num,
        begin: parseInt(
          document.querySelector("div#i2 span")?.textContent ?? "1",
          10,
        ),
        prev: "#",
        next: "#",
  ...
  const hentaifox = {
    ...
    waitVar: "g_th",
    waitFunc: () =>
      document.querySelector("#gimg")?.classList.contains("loaded"),
    run() {
      const num = parseInt(
      ...
      return {
        title: document
          ...
          .trim(),
        series: document
          .querySelector(".browse_buttons a")
          ?.getAttribute("href"),
        pages: num,
        prev: "#",
        next: "#",
  ...
  const hentaihand = {
    ...
    run() {
      ...
      return {
        ...
        series: document
          .querySelector(".reader-header h5 a")
          ?.getAttribute("href"),
        pages: images.length,
        prev: "#",
        next: "#",
  ...
  const imhentai = {
    ...
    waitVar: "g_th",
    run() {
      const galleryId = document
        .querySelector("#gallery_id")
        ?.getAttribute("value");
      const imageDir = document
        .querySelector("#image_dir")
        ?.getAttribute("value");
      const num = parseInt(
        document.querySelector("#pages")?.getAttribute("value") ?? "",
        10,
      );
      const cId = parseInt(
  ...
  const myhentaigallery = {
    ...
    run() {
      ...
      return {
        title: document.querySelector("title")?.textContent?.trim(),
        series: document
          .querySelector(".back-to-gallery a")
          ?.getAttribute("href"),
        pages: num,
        prev: "#",
        next: "#",
  ...
  const ninehentai = {
    ...
    async run() {
      const data = {
        id: parseInt(/\d+/.exec(window.location.pathname)?.at(0) ?? "0", 10),
      };
      const options = {
        method: "POST",
      ...
      const api = await fetch("/api/getBookByID", options).then(async (res) =>
        res.json(),
      );
      return {
  ...
  const pururin = {
    ...
    run() {
      const src =
        document.querySelector(".img-viewer img")?.getAttribute("src") ?? "";
      const num = [...document.querySelectorAll(".img-select option")];
      return {
        title: document.querySelector(".title")?.textContent?.trim(),
        series: document
          .querySelector(".breadcrumb-item:nth-child(4) a")
          ?.getAttribute("href"),
        pages: num.length,
        prev: "#",
        next: "#",
  ...
  const simplyhentai = {
    ...
    async run() {
      ...
      return {
        title: document
          .querySelector(".content-headline a")
          ?.textContent?.trim(),
        series: document
          .querySelector(".content-headline a")
          ?.getAttribute("href"),
    ...
    run() {
      return {
        ...
        listImages: Object.keys(unsafeWindow.readerPages.pages).map((img) =>
          unsafeWindow.readerPages.baseUriImg.replace(
            "%s",
            unsafeWindow.readerPages.pages[img].f,
          ),
        ),
      };
    },
  ...
  const tmohhentai = {
    ...
    run() {
      const num = parseInt(
        document
          .querySelector("#select-page option:last-child")
          ?.getAttribute("value") ?? "",
        10,
      );
      return {
        ...
        series: document
          .querySelector(".nav-justified li a")
          ?.getAttribute("href"),
        pages: num,
        prev: "#",
        next: "#",
  ...
  const tsumino = {
    ...
    category: "hentai",
    async run() {
      const dataopt = document
        .querySelector("#image-container")
        ?.getAttribute("data-opt");
      const datacdn =
        document.querySelector("#image-container")?.getAttribute("data-cdn") ??
        "";
  ...
  const wnacg = {
    ...
    run() {
      ...
      return {
        title: document
          .querySelector(".bread a:last-of-type")
          ?.textContent?.trim(),
        pages: pages.length,
        prev: "#",
        next: "#",
        listPages: pages.map((page) =>
          window.location.pathname.replace(/\d+/, page.value),
        ),
        img: "#picarea",
      };
    },
  ...
  function updateSettings(newValue) {
    ...
    setSettings(diffObj(settings$2, defaultSettings));
  }
  ...
  function applyZoom(
    ...
    pg.forEach((img) => {
      ...
      if (zoom === "width") {
        ...
        const nextHeight =
          window.innerHeight +
          (getUserSettings().showThumbnails || fluid ? -30 : 0);
