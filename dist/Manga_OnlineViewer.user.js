...
(function() {
  ...
  >
	...
	*/
	...
	var backgrounds = _.values(colors).map((i) => i["900"]);
	var widths = [
		400,
		600,
		900,
	...
	*/
	...
	var localhost = {
		...
		run(count) {
			const num = count ?? parseInt(/\d+/.exec(window.location.search)?.toString() ?? "5", 10);
			return {
  ...
  [data-description] {
	...
	*/
	function findElements(selector, content, matcherKey) {
		...
		return [...document.querySelectorAll(selector)].filter((element) => _.castArray(content).some((c) => matcher(element, c)));
	}
	/**
	...
	*/
	...
	var flamecomics = {
		...
		run() {
			...
			const images = _.keys(chapter?.images).map((i) => `${cdn}/${chapter?.series_id}/${chapter?.token}/${chapter?.images?.[i]?.name}?${chapter?.unix_timestamp}`);
			return {
