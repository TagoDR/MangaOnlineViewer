...
(function () {
  ...
  const bilibilicomics = {
    ...
    async run() {
      const api = await fetch(
        ...
        .then(({ data }) =>
          data.images.map((image) => `${image.path}@2000w.webp`),
        )
        .then(JSON.stringify)
        .then(async (urls) =>
          fetch(
  ...
  const mangahub = {
    ...
    async run() {
      ...
      const api = await fetch("https://mangahub.io/api/graphql", options).then(
        async (res) => res.json(),
      );
  ...
  const mangastreamwp = {
    ...
    run() {
      ...
      const images = [
        ...document.querySelectorAll("#readerarea img:not(.asurascans)"),
      ];
      return {
