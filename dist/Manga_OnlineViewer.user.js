	...
	*/
	...
	var asura = {
		...
		run() {
			...
			const imgUrls = (props.pages?.[1] || []).map((p) => p[1]?.url?.[1]).filter(Boolean);
			return {
				title: `${props.seriesName?.[1]} - Chapter ${props.chapterName?.[1]}`,
				series: `/comics/${seriesSlug}`,
				pages: imgUrls.length,
				prev: findClosestByContentEq("span", "Prev", "a")?.getAttribute("href") || document.querySelector("link[rel=\"prev\"]")?.getAttribute("href") || void 0,
				next: findClosestByContentEq("span", "Next", "a")?.getAttribute("href") || document.querySelector("link[rel=\"next\"]")?.getAttribute("href") || void 0,
				listImages: imgUrls
			};
		}
	};
