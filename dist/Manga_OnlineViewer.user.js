...
(function() {
	...
	*/
	...
	//#endregion
	//#region node_modules/@moaqzdev/toast/dist/utils.mjs
	var o$13 = "@moaqzdev/toast", i$7 = {
		_dispatchToast(s, t) {
			Object.assign(t, { type: s });
			const a = new CustomEvent(o$13, { detail: t });
	...
	*/
	...
	var k = class k {
		...
		_$AI(t, i = this) {
			t = M$1(this, t, i), a$1(t) ? t === A || null == t || "" === t ? (this._$AH !== A && this._$AR(), this._$AH = A) : t !== this._$AH && t !== E && this._(t) : void 0 !== t._$litType$ ? this.$(t) : void 0 !== t.nodeType ? this.T(t) : d$3(t) ? this.k(t) : this._(t);
		}
		O(t) {
  ...
  `,
	...
	*/
	var rulerMarkerLength = (len) => {
		if (len % 100 === 0) return 15;
		if (len % 50 === 0) return 10;
  ...
  >
	...
	*/
	function placeholder(width, height, bgColor = "#0F1C3F", textColor = "#ECEAD9") {
		return svgToUrl(rectangleRuler(width, height, bgColor, textColor));
	}
	var backgrounds = Object.values(colors).map((i) => i["900"]);
	var widths = [
		400,
		600,
		900,
	...
	*/
	...
	var fileImageExt = /.(png|jpg|jpeg|gif|bmp|webp)$/i;
	/**
	...
	*/
	var orderFiles = (a, b) => a.localeCompare(b, navigator.languages[0] || navigator.language, {
		numeric: true,
	...
	*/
	function openFileImages(evt) {
		...
		logScript("Local Files: ", files, files.map((f) => f.webkitRelativePath || f.name));
		if (input.files?.[0]) displayUploadedFiles(input.files[0].webkitRelativePath.split("/")[0] || "Local Images", files.map(URL.createObjectURL));
	}
	/**
	...
	* Copyright 2017 Google LLC
	* SPDX-License-Identifier: BSD-3-Clause
	*/ var { is: i$2, defineProperty: e$4, getOwnPropertyDescriptor: h, getOwnPropertyNames: r$3, getOwnPropertySymbols: o$8, getPrototypeOf: n$2 } = Object, a = globalThis, c = a.trustedTypes, l = c ? c.emptyScript : "", p = a.reactiveElementPolyfillSupport, d$1 = (t, s) => t, u = {
		toAttribute(t, s) {
			switch (s) {
				case Boolean:
					t = t ? l : null;
	...
	y.elementStyles = [], y.shadowRootOptions = { mode: "open" }, y[d$1("elementProperties")] = /* @__PURE__ */ new Map(), y[d$1("finalized")] = /* @__PURE__ */ new Map(), p?.({ ReactiveElement: y }), (a.reactiveElementVersions ??= []).push("2.1.2");
	//#endregion
	...
	*/
	function toPascalCase(name) {
		...
		return `Icon${(name.startsWith("Icon") ? name.substring(4) : name).split(/[-_]/).map((part) => part.charAt(0).toUpperCase() + part.slice(1)).join("")}`;
	}
	//#endregion
	...
	var svg_exports = /* @__PURE__ */ __exportAll({
		...
		IconBook: () => book_default,
		IconBookReturn: () => book_return_default,
  ...
  `;
		...
		*/
		render() {
			const swatchKeys = Object.keys(colors).filter((k) => ![
				"dark",
				"gray",
				"zinc",
				"neutral",
  ...
  `;
    ...
    `;
      ...
      <div class="swatches">
        ${(this.swatches || Object.entries(colors).filter(([name]) => ![
				"dark",
				"gray",
				"zinc",
				"neutral",
    ...
    `;
      ...
      `;
		...
		colorToHsv(color) {
			let [h, s, v] = color.to("srgb").to("hsv").coords;
			if (Number.isNaN(h)) {
				h = this.hsv.h || 0;
				s = 0;
			}
	...
	*/
	function buttonRedirectURL(event) {
		...
		if (event.button !== 1 && !event.ctrlKey) {
			...
			else if (element.id === "series") window.history.back();
		}
	}
	//#endregion
  ...
  `;
	...
	*/
	function redirectUrl(type) {
		...
		if (url && url !== "#") location.href = (0, import_dist.sanitizeUrl)(url);
		else if (type !== "next") history.back();
	}
	/**
	...
	*/
	...
	var KeybindingsPanel = class KeybindingsPanel extends i$1 {
		...
		*/
		keybindEditor() {
			const keybinds = getSettingsValue("keybinds");
			return Object.keys(keybinds).map((kb) => b$1`<label for="${kb}">${getLocaleString(kb)}:</label>
          <input
            type="text"
            class="KeybindInput"
            id="${kb}"
    ...
    `];
		...
		*/
		render() {
			...
			const loaded = Object.values(getAppStateValue("images") ?? {}).filter((i) => i.status === "loaded").length;
			return b$1`
      <nav
        id="Navigation"
        class="${e(navClasses)}"
  ...
  `;
	...
	*/
    ...
    <span id="ColorRecommendations">
      ${Object.values(sample).map((c) => b$1`<color-swatch
            .color="${c}"
            .selected=${getSettingsValue("theme")}
            @change=${changeTheme}
          ></color-swatch>`)}
  ...
  `;
	...
	*/
	var listPages = (times, begin) => sequence(times, begin).map((index) => {
...
`;
  ...
  `;
	...
	*/
	...
	//#endregion
	//#region node_modules/@moaqzdev/toast/dist/index.mjs
	var TOAST_EVENT = "@moaqzdev/toast";
	var Toaster = class Toaster extends HTMLElement {
		constructor() {
		...
		async createToast({ title: o, type: e, description: r, onConfirm: m, onCancel: f, confirmText: v = "✅", cancelText: b = "❌", duration: n }) {
			const a = this.shadowRoot.querySelector("#toast-tmpl").content.cloneNode(!0), t = {
			...
			n = this.parseDuration(n), t.title.textContent = o || "", t.container.setAttribute("data-type", e), r == null ? t.description?.remove() : t.description.textContent = r;
			const s = () => this.removeToast(t.container);
			if (e === "confirm" ? (t.confirmBtn.textContent = v, t.confirmBtn.addEventListener("click", () => {
				m?.(), s();
			}, { once: !0 }), t.cancelBtn.textContent = b, t.cancelBtn.addEventListener("click", () => {
				f?.(), s();
			}, { once: !0 })) : t.actions?.remove(), this.hasAttribute("dismissable") ? t.closeBtn.addEventListener("click", s, { once: !0 }) : t.closeBtn?.remove(), this.shadowRoot.querySelector("[data-toaster]").appendChild(a), n !== "none") {
				const c = new AbortController(), h = Date.now();
				let i = null, l = 0;
				const u = () => {
					c.abort(), s();
				};
  ...
  [data-description] {
	...
	*/
	function findElements(selector, content, matcherKey) {
		...
		if (!matcher) throw new Error(`Invalid matcherKey: ${matcherKey}`);
		return [...document.querySelectorAll(selector)].filter((element) => Array.isArray(content) ? content.some((c) => matcher(element, c)) : matcher(element, content));
	}
	/**
	...
	*/
	function findClosest(selector, content, matcherKey, ancestor = "a") {
		return findOneElement(selector, content, matcherKey)?.closest(ancestor) ?? null;
	}
	/**
	...
	*/
	var findByContentEq = (selector, content) => findElements(selector, content, "eq");
	/**
	...
	*/
	var findOneByContentStarts = (selector, content) => findOneElement(selector, content, "starts");
	/**
	...
	*/
	var findClosestByContentEq = (selector, content, ancestor = "a") => findClosest(selector, content, "eq", ancestor);
	/**
	...
	* @returns {HTMLElement | null} The matching ancestor element.
	*/
	var findClosestByContentStarts = (selector, content, ancestor = "a") => findClosest(selector, content, "starts", ancestor);
	/**
	...
	* @returns {HTMLElement | null} The matching ancestor element.
	*/
	var findClosestByContentEnds = (selector, content, ancestor = "a") => findClosest(selector, content, "ends", ancestor);
	//#endregion
	//#region src/main/asura.ts
	var asura = {
		name: "Asura Scans",
	...
	var batoto = {
		...
		run() {
			if (window.location.pathname.startsWith("/title")) {
				if (window.location.search !== "?load=2") window.location.search = "?load=2";
	...
	//#endregion
	//#region src/main/bilibilicomics.ts
	var bilibilicomics = {
		name: "BilibiliComics",
		...
		async run() {
			const images = JSON.parse(document.querySelector("#__NUXT_DATA__")?.innerHTML ?? "").filter((x) => typeof x === "string" && /.(png|jpg|jpeg|gif|bmp|webp)$/i.exec(x));
			return {
	...
	var comick = {
		...
		run() {
			...
			const pages = data?.chapter?.md_images?.map((image) => `https://meo.comick.pictures/${image?.b2key}`);
			return {
	...
	//#endregion
	//#region src/main/dysnatyscans.ts
	var dynastyscans = {
		name: "Dynasty-Scans",
	...
	//#endregion
	//#region src/main/flamecomics.ts
	var flamecomics = {
		name: "Flame Comics",
		...
		run() {
			...
			const images = Object.keys(chapter?.images).map((i) => `${cdn}/${chapter?.series_id}/${chapter?.token}/${chapter?.images?.[i]?.name}?${chapter?.unix_timestamp}`);
			return {
	...
	*/
	...
	async function collectAllSources(num) {
		...
		while (srcArray.size < num && attempts < maxScrollAttempts) {
			...
			for (const img of currentImages) {
				...
				if (src && src.length > 0) srcArray.add(src);
			}
			console.log(`Attempt ${attempts}: Collected ${srcArray.size} of ${num} sources.`);
			if (container.scrollHeight - container.clientHeight <= container.scrollTop && srcArray.size < num) {
	...
	var kagane = {
		...
		async run() {
			...
			const headers = {};
			if (rschDid) headers["X-Rsch-Did"] = rschDid;
			const seriesData = await fetch(`https://api.kagane.org/api/v1/series/${seriesId}`, { headers }).then((res) => res.json());
			const content = (await fetch(`https://api.kagane.org/api/v1/books/${seriesId}`, { headers }).then((res) => res.json())).data.content;
	...
	//#endregion
	//#region src/main/leercapitulo.ts
	var leercapitulo = {
		name: "LeerCapitulo",
	...
	//#endregion
	//#region src/main/lhtranslation.ts
	var lhtranslation = {
		name: "LHTranslation",
	...
	var mangaball = {
		...
		category: Category.MANGA,
		run: async () => {
			const script = [...document.querySelectorAll("script")].find((s) => s.textContent?.includes("chapterImages"))?.textContent;
			if (!script) return {
				title: document.querySelector("h1")?.textContent?.trim(),
			...
			const images = JSON.parse(script.match(/chapterImages\s*=\s*JSON\.parse\(\s*[`'"](.+?)[`'"]\s*\)/)?.[1] ?? script.match(/chapterImages\s*=.*(\[.*?\])/)?.[1] ?? "[]");
			const csrfToken = document.querySelector("meta[name=\"csrf-token\"]")?.getAttribute("content") || "";
			const allChapters = ((await (await fetch("/api/v1/chapter/chapter-listing-by-title-id/", {
				method: "POST",
				headers: {
			...
			})).json()).ALL_CHAPTERS || []).sort((a, b) => b.number_float - a.number_float);
			const findChapter = (direction) => {
				...
				for (let i = start; i >= 0 && i < allChapters.length; i += step) {
					const trans = allChapters[i].translations.find((t) => t.language === chapterLanguage && String(t.volume) === volumeStr);
					if (trans) return trans.url;
				}
			};
			return {
				title: document.querySelector("h1")?.textContent?.trim(),
	...
	var mangadex = {
		...
		async run() {
			const home = `https://api.mangadex.org/at-home/server/${/\/chapter\/([^/]+)(\/\d+)?/.exec(window.location.pathname)?.at(1)}`;
	...
	var mangago = {
		...
		run() {
			...
			return {
				...
				listImages: images,
				before() {
					if (images.some((s) => s === "")) document.querySelector("#nform")?.submit();
				}
			};
		}
	};
	//#endregion
	//#region src/main/mangahub.ts
	var mangahub = {
		name: "MangaHub",
		...
		async run() {
			...
			const data = { query: `{chapter(x:m01,slug:"${unsafeWindow.CURRENT_MANGA_SLUG ?? window.location.pathname.split("/")[2]}",number:${window.location.pathname.split("/")[3].replace("chapter-", "")}){pages}}` };
			const options = {
				method: "POST",
			...
			const api = await fetch("https://api.mghcdn.com/graphql", options).then(async (res) => res.json());
	...
	//#endregion
	//#region src/main/mangakakalot.ts
	var mangakakalot = {
		name: [
			"MangaKakalot",
			"NeloManga ",
		...
		run() {
			...
			return {
				title: document.querySelector(".info-top-chapter h2, .imageOptions-chapter-info-top h1, .panel-chapter-info-top h1")?.textContent?.trim(),
	...
	//#endregion
	//#region src/main/mangareader.ts
	var mangareader = {
		name: "MangaReader",
		...
		async run() {
			...
			const src = [...document.querySelectorAll(".ds-image[data-url], .iv-card[data-url]")].map(async (img) => {
				const url = img.getAttribute("data-url") ?? "";
				if (url && img.classList.contains("shuffled")) return (await imgReverser(url)).toDataURL();
				return url;
			});
			return {
	...
	//#endregion
	//#region src/main/mangastreamwp.ts
	var mangastreamwp = {
		name: [
		...
		run() {
			...
			return {
				...
				pages: images.length,
				prev: findByContentEq(":where(.nextprev, .inner_nPL) a", ["Prev", "Anterior"])?.[0]?.getAttribute("href"),
				next: findByContentEq(":where(.nextprev, .inner_nPL) a", ["Next", "Siguiente"])?.[0]?.getAttribute("href"),
				listImages: images.map((img) => img.getAttribute("data-src") ?? img.getAttribute("data-lazy-src") ?? img.getAttribute("src") ?? "")
			};
		}
	};
	//#endregion
	//#region src/main/mangatoon.ts
	var mangatoon = {
		name: "MangaToons",
	...
	var mangatown = {
		...
		run() {
			...
			const images = [...document.querySelectorAll(".read_img .image")].map((img) => img.getAttribute("src") ?? "");
			...
			return {
				...
				...images.length > 1 ? { listImages: images } : {
					listPages: pages,
					img: "#image"
				}
			};
	...
	var manhwaweb = {
		...
		async run() {
			...
			const api = await fetch(`https://manhwawebbackend-production.up.railway.app/chapters/see${slug}`).then(async (res) => res.json());
			const data = await fetch(`https://manhwawebbackend-production.up.railway.app/chapters/seeprevpost${slug}`).then(async (res) => res.json());
			return {
	...
	//#endregion
	//#region src/main/olympusbiblioteca.ts
	var olympusbiblioteca = {
	...
	//#endregion
	//#region src/main/readcomicsonline.ts
	...
	//#endregion
	//#region src/main/reaperscans.ts
	var reaperscans = {
		name: "ReaperScans",
	...
	//#endregion
	//#region src/main/vortexscans.ts
	var vortexscans = {
		name: "Vortex Scans",
		...
		},
		{
			...
			run() {
				...
				return {
					...
					listImages: images.map((img) => img.getAttribute("data-url") ?? img.getAttribute("data-src") ?? img.getAttribute("src") ?? "")
				};
			}
		},
		{
