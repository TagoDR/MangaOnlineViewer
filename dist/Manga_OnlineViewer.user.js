...
(function () {
  ...
  function logScriptVerbose(...text) {
    if (["dev", "development"].includes("main"))
  ...
  * {
  ...
  function findElements(selector, content, matcherKey) {
    ...
    return [...document.querySelectorAll(selector)].filter(
      (element) => Array.isArray(content) ? content.some((c) => matcher(element, c)) : matcher(element, content)
    );
  }
  ...
  const findClosestByContentEq = (selector, content, ancestor = "a") => findClosest(selector, content, "eq", ancestor);
  const findClosestByContentStarts = (selector, content, ancestor = "a") => findClosest(selector, content, "starts", ancestor);
  const findClosestByContentEnds = (selector, content, ancestor = "a") => findClosest(selector, content, "ends", ancestor);
  ...
  const flamecomics = {
    ...
    run() {
      ...
      const images = Object.keys(chapter?.images).map(
        (i) => `${cdn}/${chapter?.series_id}/${chapter?.token}/${chapter?.images?.[i]?.name}?${chapter?.unix_timestamp}`
      );
      return {
  ...
  const ikigai = {
    ...
    category: Category.MANGA,
    run() {
      const images = document.querySelector('script[type="qwik/json"]')?.textContent?.match(/http[^'"]+webp/gi) ?? [];
      return {
  ...
  const mangabuddy = {
    ...
    waitVar: "chapImages",
    run() {
      const images = unsafeWindow.chapImages.split(",").map((s) => new URL(s).pathname.replace("/res/", "https://sb.mbcdn.xyz/"));
      return {
  ...
  const mangatown = {
    ...
    run() {
      ...
      const pages = [...options ?? []].map((element) => element.value).filter((url) => !url.endsWith("featured.html")).map((url) => new URL(url, window.location.href).href);
      return {
        ...
        prev: document.querySelector(".chapter_select option:checked")?.previousElementSibling?.getAttribute("value"),
        next: document.querySelector(".chapter_select option:checked")?.nextElementSibling?.getAttribute("value"),
  ...
  const vortexscans = {
    ...
    run() {
      const data = unsafeWindow.__next_f.find(
        (i) => /images/.test(i?.[1])
      )?.[1];
