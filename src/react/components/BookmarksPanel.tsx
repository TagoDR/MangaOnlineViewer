...
function BookmarksPanel() {
  ...
  return (
    <div id="BookmarksPanel" className="panel">
      <button
        id="CloseBookmarks"
      ...
      <div id="BookmarksList">
        {isEmpty(s.bookmarks)
          ...
          : s.bookmarks.map((mark, index) => (
              <div id={`Bookmark${index + 1}`} className="BookmarkItem" key={mark.url}>
                <span className="bookmarkColumnLarge">
                  <span className="bookmarkName">{mark.name}</span>
                  <br />
                  <a className="bookmarkURl" href={mark.url} target="_blank" rel="noreferrer">
                    {mark.url}
                  </a>
                </span>
                ...
                <span className="bookmarkFunctions">
                  <a href={mark.url} target="_blank" rel="noreferrer">
                    <button className="ControlButton open" title="Open Bookmark" type="button">
                      <IconExternalLink />
                    </button>
                  </a>
                  <button
