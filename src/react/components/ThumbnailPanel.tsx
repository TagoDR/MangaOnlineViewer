...
import { transformScrollToHorizontal } from '../../ui/events/common';
...
function ThumbnailPanel({ manga }: Props) {
  ...
  return (
    <nav id="Navigation" className={`panel ${s.showThumbnails ? '' : 'disabled'}`}>
      ...
      <div id="Thumbnails" onWheel={transformScrollToHorizontal}>
        {Array.from(Array(manga.pages + 1).keys())
          .slice(manga.begin)
          .map((index) => (
            <Thumbnail index={index} key={index} />
          ))}
      </div>
    </nav>
  );
