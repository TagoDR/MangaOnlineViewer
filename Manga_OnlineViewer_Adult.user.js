  ...
  var ninehentai = {
    ...
    run() {
      ...
      return {
        ...
        prev: '#',
        next: '#',
        listImages: [...Array(api.total_page).keys()].map((i) => `${api.image_server + api.id}/${i + 1}.jpg`),
      };
    },
  };
  ...
  var exhentai = {
    ...
    run() {
      ...
      return {
        ...
        lazy: true,
        bruteForce(func) {
          [...Array(maxGalley).keys()].slice(Math.floor(Math.abs((func.begin - 1) / 40))).map((galleryId, galleryOrder) =>
            func.getHtml(galleryId > 0 ? `${gallery}?inline_set=ts_m&p=${galleryId}` : `${gallery}?inline_set=ts_m`, func.wait * galleryOrder).then((html) => {
              $(html)
                .find('.gdtm a, .gdtl a')
                .get()
  ...
  var hbrowse = {
    ...
    run() {
      ...
      return {
        ...
        pages: num,
        prev: chapter.eq(chapter.index(chapter.filter(`[href='${W.location.href}']`)) - 1).attr('href'),
        next: chapter.eq(chapter.index(chapter.filter(`[href='${W.location.href}']`)) + 1).attr('href'),
  ...
  var hentaihere = {
    ...
    run() {
      ...
      return {
        ...
        prev: '#',
        next: '#',
        listImages: [...Array(num).keys()].map((i) => src.replace(/[0-9]+.jpg/, String(`00000${i + 1}`).slice(-1 * size) + ext)),
      };
    },
  };
  ...
  var kingcomix = {
    ...
    run() {
      const src = $('figure img, .entry-content img.lazy')
        .get()
        .map((i) => $(i).attr('src') || $(i).attr('data-src') || $(i).attr('data-full-url') || $(i).attr('data-lazy-src'));
      return {
  ...
  var myhentaigallery = {
    ...
    run() {
      ...
      return {
        ...
        prev: '#',
        next: '#',
        listImages: [...Array(num).keys()].map((i) => src.replace(/[0-9]+\./, `${String(`000${i + 1}`).slice(-3)}.`)),
      };
    },
  };
