...
(function () {
  ...
  var hbrowse = {
    ...
    run() {
      const url =
        W.location.href + (W.location.href.slice(-1) === '/' ? '' : '/');
    ...
    run() {
      ...
      return {
        ...
        listImages: [...Array(num).keys()].map((i) =>
          src.replace(
            /[0-9]+.jpg/,
            String(`00000${i + 1}`).slice(-1 * size) + ext
          )
        ),
      };
    },
  ...
  var myhentaigallery = {
    ...
    run() {
      ...
      const num = parseInt(
        $('.pagination ul li:not(.next,.last):last').text(),
        10
      );
      return {
        title: $('title').text().trim(),
  ...
  var tsumino = {
    ...
    run() {
      ...
      $.ajax({
        type: 'GET',
        url: `https://www.tsumino.com/Read/Load?q=${$('#image-container').attr(
          'data-opt'
        )}`,
        dataType: 'json',
        async: false,
  ...
  var vercomicsporno = {
    ...
    run() {
      ...
      const src = imgs.map(
        (i) => $(i).attr('data-lazy-src') || $(i).attr('src')
      );
      return {
